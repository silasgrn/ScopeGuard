        ...
        FirewallBackend::Nftables => {
            if status.rules_loaded {
                vec![Finding {
                    ...
                    description: "The host has an nftables firewall backend configured."
                        .to_string(),
                    risk: "A firewall backend is present but may still have permissive rules."
                        .to_string(),
                    recommendation:
                        "Review nftables policies and ensure default deny rules are enforced."
                            .to_string(),
                ...
                vec![Finding {
                    ...
                    description:
                        "The nftables backend is installed, but the ruleset could not be read."
                            .to_string(),
                    risk: "Missing firewall rules may expose the host to network attacks."
                        .to_string(),
                    recommendation:
                        "Verify nftables configuration and permissions for ruleset inspection."
                            .to_string(),
        ...
        FirewallBackend::Iptables => {
            if status.rules_loaded {
                ...
                vec![Finding {
                    ...
                    description:
                        "The iptables backend is available, but its rules could not be inspected."
                            .to_string(),
                    risk: "Missing iptables visibility may hide misconfigurations.".to_string(),
                    recommendation:
                        "Ensure iptables is installed and accessible to the current user."
                            .to_string(),
                    severity: Severity::Medium,
                    category: "Firewall".to_string(),
                }]
